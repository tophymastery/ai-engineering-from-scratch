...
// identity-auth / merchant-catalog / cart / pricing-promo / order so all slice
// IDs share one format.
...
// ms timestamp + 80 random bits>. Prefixes used here: pay_ (payment), evt_
// (event), txn_ (payment ledger entry), wal_ (wallet).
