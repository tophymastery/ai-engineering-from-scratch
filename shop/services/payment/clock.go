...
// ever reads wall time"). Time is load-bearing for this slice's deterministic
// fixtures (the circuit-breaker cooldown, event/webhook occurred_at stamps), so
// it is injected, never read from the wall in a test — the failover-storm and
// webhook-replay tests advance a frozen clock, they never sleep.
//
// Mirrors services/order/clock.go and services/pricing-promo/clock.go.
