...
// resources (menus, carts) → 412 on stale write"). The ETag is a STRONG, opaque
// validator derived from the cart id + its monotonic version. Every accepted
// add/remove bumps the version, so the ETag a reader holds becomes stale the
// instant anyone else's write commits — which is exactly what makes a concurrent
// stale write detectable and rejectable with 412. This is the SAME primitive
// V-T3 (merchant-catalog) proved for menus; cart reuses it verbatim.

// makeETag derives the strong ETag for a cart at a given version. It is a short
// SHA-256 over "cart:id:version" wrapped in the required double quotes
