...
# V-T6 added services/feed-cache — a shared-lib change fans out to all of them.
got="$(printf '%s\n' 'libs/errors/errors.go' | "$CP" --stdin)"
want="$(printf '%s\n' 'bffs/factories-ts' 'gateway' 'services/_placeholder' 'services/fakes' 'services/feed-cache' 'services/identity-auth' 'services/identity-profile' 'services/merchant-catalog' 'services/ranking' 'services/search-indexer' 'services/search-query' | sort -u)"
