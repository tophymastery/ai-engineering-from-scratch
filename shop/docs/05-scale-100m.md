...
**D7: CQRS read models for second access paths.** Merchant order queue (sharded by `merchant_id`, built from `order.*` events, freshness p99 < 2 s), ops/admin order search (OpenSearch headers index), customer history (shard-local). Consequence: projections must be rebuildable from the event store (tested, V-T34).
...
**D31: Backend language = Rust, end-to-end (supersedes the Go/TypeScript stack wording in README, 01, 03, 04).** Every backend component — all microservices **and** all BFFs — is implemented in Rust: async on **tokio**, **axum** for HTTP, **tonic** for gRPC, **sqlx** for PostgreSQL, **rdkafka** for Kafka, **opentelemetry-rust** for the telemetry plane (doc 04 §2). The monorepo is a single **Cargo workspace**; `libs/*` are crates, `services/*` and `bffs/*` are member crates; path-based change detection maps to the workspace dependency graph exactly as before. Rationale: memory safety without a garbage collector, which removes GC pauses as a tail-latency source under the §4 p99 budgets (checkout p99 < 800 ms, dispatch p95 < 5 s, tracking freshness < 10 s); **data-race freedom enforced at compile time** by `Send`/`Sync` (so the correctness a Go `-race` run bought is a build-time guarantee, with `loom` model-checking reserved for hand-rolled lock-free code); one toolchain (`cargo`) and one artifact story across the whole backend. Testing (doc 03): `cargo test`/`cargo nextest`, `#[tokio::test]` for async, `testcontainers-rs` for real PG/Kafka/Redis, determinism via an injected `Clock` + seeded `Rng` (01 §6). BFFs move from NestJS to **axum** to keep one language across the backend; the TypeScript factory mirror is retained **only** for frontend test-data generation. This decision changes the language, not the architecture — service boundaries, event contracts, sagas, idempotency (D9), sharding (D11), and every other decision D1–D30 stand unchanged.
